...
class FrxeDownloadService : Service() {
    ...
    private lateinit var pipeline: DownloadPipeline
    ...
    override fun onCreate() {
        ...
        pipeline = DownloadPipeline(applicationContext)
    ...
    override fun onDestroy() {
        ...
        if (runningId != null && DownloadQueueStore.item(runningId)?.state == DownloadQueueItemState.Running) {
            DownloadQueueStore.fail(
                runningId,
                "Download interrupted; reopen Frxe to resume"
            )
        }
    ...
    override fun onTimeout(startId: Int, fgsType: Int) {
        activeItemId?.let { id ->
            if (DownloadQueueStore.item(id)?.state == DownloadQueueItemState.Running) {
                DownloadQueueStore.fail(
                    id,
                    "Android paused this long-running download. Open Frxe to retry."
                )
            }
        }
        pipeline.cancel()
    ...
    private suspend fun runItem(itemId: String) {
        ...
        val request = item.toSaveRequest()
            ?: run {
                DownloadQueueStore.fail(itemId, "Invalid queued download request")
                return
            }

        try {
            val result = pipeline.save(request) { state ->
    ...
    private fun DownloadQueueItem.toSaveRequest(): SaveRequest? {
        val parsedFormat = runCatching { SaveFormat.valueOf(format) }.getOrNull() ?: return null
        val parsedQuality = runCatching { SaveQuality.valueOf(quality) }.getOrNull() ?: return null
